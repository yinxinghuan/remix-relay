...
Remix Relay 是一个 `social` 类型的移动端小游戏：AlterU 社交 · 接力变形。刷到一张正在接力的超现实图，tap 一个变形动作（点燃/沉水/化金/霓虹…），AI 用 img2img 接出你的版本，你成新链尖、上家收到通知。一张永远在变形的集体图，remix 即社交动词。接力墙看最近链尖，点任意一张接着变。

## 2. Visual Design

- 整体布局：页面占用 100vw x 100vh，主体验居中，HUD 与操作区覆盖在游戏层上方，移动端以单手操作为优先。
- 背景与配色：主要颜色使用 #fff、#000、#07060d、#ece9ff、#f3eefe、#9b95bf、#ff3d9a、#34e3ff；高亮元素用于可点击目标、得分、结果或稀有状态。
- 字体：使用 'Inter', system-ui, sans-serif、'Inter', system-ui, -apple-system, sans-serif、'Montserrat', 'Inter', sans-serif，按钮与状态文字保持 12-24 px 的可读范围。
...
- seed-5.webp：位图图片，用于角色、场景、封面、反馈或品牌视觉。
- seed-2.webp：位图图片，用于角色、场景、封面、反馈或品牌视觉。
- seed-3.webp：位图图片，用于角色、场景、封面、反馈或品牌视觉。
